...
void xtouch_firmware_onProgress(size_t currSize, size_t totalSize)
{
    ...
    lv_label_set_text_fmt(introScreenCaption, LV_SYMBOL_CHARGE " Обновление %d%%", progress);
    lv_timer_handler();
    lv_task_handler();
}
...
void xtouch_firmware_checkFirmwareUpdate(void)
{
    ...
    if (xtouch_filesystem_exist(SD, xtouch_paths_firmware_ota_fw))
    {
        ...
        if (updateSucceeded)
        {
            ...
            lv_label_set_text(introScreenCaption, LV_SYMBOL_OK " Обновление завершено");
            lv_timer_handler();
            lv_task_handler();
        }
        else
        {
            printf(" Update error\n");
            lv_label_set_text(introScreenCaption, LV_SYMBOL_WARNING " Ошибка обновления");
            lv_timer_handler();
            lv_task_handler();
            delay(3000);
            printf(" Deleting firmware file\n");
                    lv_label_set_text(introScreenCaption, LV_SYMBOL_TRASH " Удаление файла прошивки");
            lv_timer_handler();
            lv_task_handler();
        }
