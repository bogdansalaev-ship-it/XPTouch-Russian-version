...
void xtouch_cloud_mqtt_connect()
{
    xtouch_mqtt_connect(cloud.getUsername().c_str(), cloud.getAuthToken().c_str(),
                        LV_SYMBOL_CHARGE " Подключение к облачному MQTT", true);
}

void xtouch_local_mqtt_connect()
{
    xtouch_mqtt_connect("bblp", xTouchConfig.xTouchAccessCode,
                        LV_SYMBOL_CHARGE " Подключение к принтеру", false);
}
...
{
    lv_label_set_text(introScreenCaption, LV_SYMBOL_CHARGE " Подключение к BBL Cloud");
    lv_timer_handler();
    lv_task_handler();
    delay(32);
...
{
    lv_label_set_text(introScreenCaption, LV_SYMBOL_CHARGE " Подключение к принтеру");
    lv_timer_handler();
    lv_task_handler();
    delay(32);
