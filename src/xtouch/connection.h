...
bool xtouch_wifi_setup()
{
    ...
    if(xtouch_filesystem_exist(SD, xtouch_paths_provisioning)){
        cloud_mode = true;
        lv_label_set_text(introScreenCaption, wifiConfig.isNull() ? LV_SYMBOL_SD_CARD " Режим настройки" : LV_SYMBOL_WARNING " Некорректные provisioning.json и xtouch.json");
        ...
        lv_label_set_text(introScreenCaption, wifiConfig.isNull() ? LV_SYMBOL_SD_CARD " LAN-режим" : LV_SYMBOL_WARNING " Некорректный xtouch.json");
        lv_obj_set_style_text_color(introScreenCaption, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_timer_handler();
        lv_task_handler();
        ...
        lv_label_set_text(introScreenCaption, wifiConfig.isNull() ? LV_SYMBOL_SD_CARD " Отсутствуют provisioning.json и xtouch.json" : LV_SYMBOL_WARNING " Некорректные provisioning.json и xtouch.json");
    ...
    lv_label_set_text(introScreenCaption, LV_SYMBOL_WIFI " Подключение");
    ...
    lv_label_set_text(introScreenCaption, LV_SYMBOL_WIFI " Подключение");
        ...
        lv_label_set_text(introScreenCaption, LV_SYMBOL_WARNING " Повтор WiFi...");
    ...
    delay(1000);
    lv_label_set_text(introScreenCaption, LV_SYMBOL_WIFI " Подключено");
    lv_timer_handler();
    lv_task_handler();
    delay(1000);
