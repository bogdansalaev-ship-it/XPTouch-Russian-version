...
void xtouch_events_onOtaUpdateNow(lv_msg_t *m)
{
    ...
    lv_label_set_text_fmt(introScreenCaption, " Остановка веб-сервера ");
    lv_timer_handler();
    lv_task_handler();
    ...
    {
        lv_label_set_text_fmt(introScreenCaption, " Отключение MQTT ");
        lv_timer_handler();
        lv_task_handler();
...
void xtouch_setupGlobalEvents()
{
    ...
    // lv_msg_subscribe(XTOUCH_SETTINGS_NEOPIXEL_SET, (lv_msg_subscribe_cb_t)xtouch_events_onNeoPixelBrightnessSet, NULL);
    ...
    lv_msg_subscribe(XTOUCH_OPTIONAL_NEOPIXEL_SET, (lv_msg_subscribe_cb_t)xtouch_events_onNeoPixelBrightnessSet, NULL);
