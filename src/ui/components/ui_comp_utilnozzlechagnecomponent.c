...
lv_obj_t *ui_utilNozzleChagneComponent_create(lv_obj_t *comp_parent)
{
    ...
    lv_label_set_text_fmt(cui_nozzleTitle, LV_SYMBOL_SETTINGS " ВЫБОР НОЗЛА");
    ...
    lv_dropdown_set_options(cui_nozzleTypeSelectDropDown, "закалённая сталь\nнержавеющая сталь");
    ...
    lv_label_set_text(cui_saveNozzleButton, LV_SYMBOL_SAVE " Сохранить");
    ...
    lv_obj_add_event_cb(cui_saveNozzleButton, ui_event_comp_utilNozzleChagneComponent_onSaveButtonClick, LV_EVENT_CLICKED, children);

    lv_obj_add_event_cb(cui_nozzleTypeSelectDropDown, ui_event_comp_utilNozzleChagneComponent_nozzleTypeDropDown, LV_EVENT_VALUE_CHANGED, children);
    lv_obj_add_event_cb(cui_nozzleDemiliterSelectDropDown, ui_event_comp_utilNozzleChagneComponentt_nozzleDemiliterDropDown, LV_EVENT_VALUE_CHANGED, children);
