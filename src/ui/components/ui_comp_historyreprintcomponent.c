...
static void populate_printer_dropdown(lv_obj_t *dd)
{
    ...
    for (int di = 0; di < s_printer_dd_count; di++)
    {
        ...
        if (slot == 0)
        {
            strlcpy(buf + len, xTouchConfig.xTouchPrinterName[0] ? xTouchConfig.xTouchPrinterName : "Этот принтер", sizeof(buf) - len);
        }
        else
        {
            int oi = slot - 1;
            const other_printer_status_t *p = &otherPrinters[oi];
            strlcpy(buf + len, p->name[0] ? p->name : "Принтер", sizeof(buf) - len);
        }
    }
...
static void populate_summary_panel(void)
{
    ...
    char info[96];
    if (t->device_name[0])
        snprintf(info, sizeof(info), "%s  |  Пластина %d", t->device_name, t->plate_index);
    else
        snprintf(info, sizeof(info), "Пластина %d", t->plate_index);
                             ...
                             int ams_id, int tray_id, char *tt, int is_loaded, int is_selectable)
{
    ...
    char line[56];
    if (is_loaded)
        snprintf(line, sizeof(line), "AMS-%c Слот-%d %s", ul, tray_id + 1, (tt && tt[0]) ? tt : "?");
    else
        snprintf(line, sizeof(line), "AMS-%c Слот-%d X", ul, tray_id + 1);
...
static void rebuild_right_selector(void)
{
    ...
    snprintf(title_buf, sizeof(title_buf), "Цвет-%d %s", mi + 1, (mapm && mapm->filamentType[0]) ? mapm->filamentType : "");
    ...
    lv_label_set_text(set_lbl, "УСТАНОВИТЬ");
...
lv_obj_t *ui_historyReprintComponent_create(lv_obj_t *comp_parent)
{
...

    {
        ...
        s_title_lbl = title;
        lv_label_set_text(title, "Повтор");
    ...
    lv_obj_set_style_pad_bottom(form, 4, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    for (int mi = 0; mi < nrows; mi++)
    {
        ...
        else
        {
            ...
            if (map_cnt < 0)
                lv_label_set_text(lab, "Загрузка...");
            else
                lv_label_set_text(lab, "Нет данных о филаменте в задаче.");
