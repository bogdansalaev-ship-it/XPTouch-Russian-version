...
static lv_obj_t *create_tab_button(lv_obj_t *parent, const char *label, int index)
{
    ...
    lv_obj_set_height(btn, lv_pct(100));  /* 親の高さいっぱい */
...
lv_obj_t *ui_utilTabbarComponent_create(lv_obj_t *comp_parent, int active_tab_index)
{
    ...
    children[UI_COMP_UTILTABBARCOMPONENT_BTN_TEMP]    = create_tab_button(cui, "ТЕМП",    0);
    children[UI_COMP_UTILTABBARCOMPONENT_BTN_AXIS]   = create_tab_button(cui, "ОСИ",    1);
    children[UI_COMP_UTILTABBARCOMPONENT_BTN_HOTEND] = create_tab_button(cui, "ГОЛОВКА",  2);
    children[UI_COMP_UTILTABBARCOMPONENT_BTN_UTIL]    = create_tab_button(cui, "УТИЛ",    3);
