...
lv_obj_t *ui_setupTabbarComponent_create(lv_obj_t *comp_parent, lv_obj_t *general_panel, lv_obj_t *option_panel, lv_obj_t *system_panel, int active_tab_index)
{
    ...
    children[UI_COMP_SETUPTABBARCOMPONENT_BTN_GENERAL] = create_tab_button(cui, "Общее", 0, cui);
    children[UI_COMP_SETUPTABBARCOMPONENT_BTN_OPTION]  = create_tab_button(cui, "Опции",  1, cui);
    children[UI_COMP_SETUPTABBARCOMPONENT_BTN_SYSTEM]  = create_tab_button(cui, "Система",  2, cui);
