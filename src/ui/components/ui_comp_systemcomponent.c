...
lv_obj_t *ui_systemComponent_create(lv_obj_t *comp_parent)
{
    ...
    lv_label_set_text(cui_pairingTitle, LV_SYMBOL_LIST " ПОДКЛЮЧЕННЫЕ ПРИНТЕРЫ");
    ...
    {
        lv_label_set_text_fmt(cui_unpairButton, LV_SYMBOL_SHUFFLE " Отвязать [ %s ]", xTouchConfig.xTouchPrinterName);
    }
    ...
    lv_label_set_text(cui_settings_otaLabel, LV_SYMBOL_DOWNLOAD "OTA обновление");
    ...
    lv_label_set_text(cui_otaNowButton, LV_SYMBOL_DOWNLOAD " Обновить сейчас");
    ...
    lv_label_set_text(cui_reseDeviceButton, LV_SYMBOL_POWER " Перезагрузить устройство");
