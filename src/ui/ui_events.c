...
void onHomeControllerPlayPause(lv_event_t *e)
{
    ...
    case XTOUCH_PRINT_STATUS_PREPARE:
        ui_confirmPanel_show(LV_SYMBOL_WARNING " Приостановить печать?", onHomeControllerPlayPauseConfirm);
        break;
    }
...
{
    ui_confirmPanel_show(LV_SYMBOL_WARNING " Отменить печать?", onHomeControllerStopConfirm);
}
...
{
    ui_confirmPanel_show(LV_SYMBOL_WARNING " ОБНОВЛЕНИЕ", onSettingsOtaUpdateNowOnYES);
}
...
void onPrintersPause(lv_event_t *e)
{
    ...
    {
        _printers_slot = slot;
        ui_confirmPanel_show(LV_SYMBOL_WARNING " Приостановить печать?", onPrintersPauseConfirm);
    }
}
...
void onPrintersStop(lv_event_t *e)
{
    ...
    ui_confirmPanel_show(LV_SYMBOL_WARNING " Отменить печать?", onPrintersStopConfirm);
}
...
{
    ui_confirmPanel_show(LV_SYMBOL_WARNING " ПЕРЕЗАГРУЗКА", onSettingsResetDeviceConfirm);
}

void onSettingsUnPairConfirm() { lv_msg_send(XTOUCH_SETTINGS_UNPAIR, NULL); }
void onSettingsUnPair(lv_event_t *e)
{
    ui_confirmPanel_show(LV_SYMBOL_WARNING " Отвязать принтер", onSettingsUnPairConfirm);
}

void onSettingsWOP(lv_event_t *e)
{
    if (xtouch_settings_initializing) return;
...
{
    if (xtouch_settings_initializing) return;
    ui_confirmPanel_show(LV_SYMBOL_WARNING " Переключить LCD\n" LV_SYMBOL_REFRESH " Перезагрузка", onSettingsTFTFlipConfirm);
}
...
void onFilamentUnload(lv_event_t *e)
{
    if (bambuStatus.m_tray_now < 16)
    {
        ...
        ui_confirmPanel_show("Удалите филамент\nпосле\n" LV_SYMBOL_CUT, onFilamentUnloadConfirm);
}

void onFilamentLoadConfirm() { lv_msg_send(XTOUCH_COMMAND_LOAD_FILAMENT, NULL); }
void onFilamentLoad(lv_event_t *e)
{
    ui_confirmPanel_show(LV_SYMBOL_PLAY " Загрузите новый филамент\n" LV_SYMBOL_PLAY " в принтер\n\n" LV_SYMBOL_OK " Нажмите ДА для продолжения", onFilamentLoadConfirm);
}
...
{
    _slot = slot;
    ui_confirmPanel_show(LV_SYMBOL_PLAY " Загрузите новый филамент\nиз слота AMS\n" LV_SYMBOL_PLAY " в принтер\n\n" LV_SYMBOL_OK " Нажмите ДА для продолжения", onAmsSlotLoadConfirm);
}
