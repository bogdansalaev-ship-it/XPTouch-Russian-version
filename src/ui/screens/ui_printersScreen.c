...
static const char *print_status_str(int s)
{
    ...
    case XTOUCH_PRINT_STATUS_IDLE: return "ОЖИДАНИЕ";
    case XTOUCH_PRINT_STATUS_RUNNING: return "Печать";
    case XTOUCH_PRINT_STATUS_PAUSED: return "Пауза";
    case XTOUCH_PRINT_STATUS_FINISHED: return "Завершено";
    case XTOUCH_PRINT_STATUS_PREPARE: return "Подготовка";
    case XTOUCH_PRINT_STATUS_FAILED: return "Ошибка";
    default: return "-";
    }
}
...
static void update_one_row(int slot, lv_obj_t *row)
{
    ...
    {
        if (tot_layers > 0)
            snprintf(layerBuf, sizeof(layerBuf), "Слой %d/%d | %s", cur_layer, tot_layers, timeBuf);
        else
            snprintf(layerBuf, sizeof(layerBuf), "Слой - | %s", timeBuf);
    }
    else
    {
